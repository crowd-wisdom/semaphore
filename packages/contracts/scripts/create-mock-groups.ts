...
async function main() {
    ...
    let tx = await semaphoreContract["createGroup(address)"](adminAddress)
    ...
    // Remove the third member.
    {
        ...
        tx = await semaphoreContract.removeMember(groupId, members[2], siblings)
        await tx.wait()
    }
    ...
    // Update the second member.
    {
        ...
        tx = await semaphoreContract.updateMember(groupId, members[1], members[2], siblings)
        await tx.wait()
    }
